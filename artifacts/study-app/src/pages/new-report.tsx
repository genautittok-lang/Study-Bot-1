...
export default function NewReport() {
  ...
  if (step === "generating") {
    return (
      <div className="px-4 pt-16 pb-4 flex flex-col items-center justify-center min-h-[75vh]">
        ...
        <p className="text-[12px] text-muted-foreground/60 mb-6">{t("generatingDesc")}</p>
        <div className="w-56 h-1.5 bg-muted rounded-full overflow-hidden">
          <motion.div className="h-full progress-fill" animate={{ width: `${progress}%` }} transition={{ duration: 0.5 }} />
        </div>
      </div>
    );
  }
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pt-6 pb-4">
        <div className="flex items-center gap-3.5 mb-5">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center pop-in" style={{ background: "hsl(160 60% 40% / 0.08)" }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="hsl(160 60% 40%)" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
          </div>
          <div>
            <h2 className="text-base font-bold text-foreground">{t("done")}</h2>
            <p className="text-[12px] text-muted-foreground/60">{selectedType?.label} · {wordCount.toLocaleString()}</p>
          </div>
        </div>
        <div className="flex gap-2.5 mb-5">
          <motion.button whileTap={{ scale: 0.96 }}
            onClick={() => { navigator.clipboard.writeText(result); hapticSuccess(); }}
            className="flex-1 btn-primary py-3 text-[13px] font-semibold flex items-center justify-center gap-2">
          ...
          <motion.button whileTap={{ scale: 0.96 }} onClick={resetForm}
            className="flex-1 btn-secondary py-3 text-[13px] font-semibold">
            {t("newOne")}
          </motion.button>
        </div>
  ...
  if (step === "error") {
    return (
      <div className="px-4 pt-16 pb-4 flex flex-col items-center justify-center min-h-[75vh]">
        ...
        <button onClick={() => setStep("details")} className="btn-primary px-6 py-2.5 text-[13px] font-semibold">{t("tryAgain")}</button>
      </div>
    );
  }
  ...
  if (step === "type") {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pt-6 pb-4">
        <h2 className="text-xl font-bold text-foreground tracking-tight mb-0.5">{t("newDocument")}</h2>
        <p className="text-[13px] text-muted-foreground/60 mb-4">{t("chooseDocType")}</p>
        <StepBar step={step} />
        <div className="space-y-2">
          {REPORT_TYPES.map((type, i) => (
            ...
            >
              <div className="w-11 h-11 rounded-[14px] flex items-center justify-center shrink-0 text-lg" style={{ background: "hsl(var(--primary) / 0.06)" }}>{type.icon}</div>
              <div className="flex-1">
                <div className="font-semibold text-[14px] text-foreground">{type.label}</div>
                <div className="text-[12px] text-muted-foreground/50 mt-0.5">{type.desc}</div>
              </div>
  ...
  if (step === "category") {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pt-6 pb-4">
        ...
        <h2 className="text-xl font-bold text-foreground tracking-tight mb-0.5">{t("chooseCategory")}</h2>
        <p className="text-[13px] text-muted-foreground/60 mb-4">{CATEGORIES.reduce((a, c) => a + c.subjects.length, 0)}+ {t("allSubjects").toLowerCase()}</p>
        <StepBar step={step} />
        <div className="grid grid-cols-2 gap-2.5">
          {CATEGORIES.map((cat, i) => (
            ...
            >
              ...
              <div className="text-[11px] text-muted-foreground/50 mt-0.5">{cat.subjects.length} {t("allSubjects").toLowerCase()}</div>
            </motion.button>
          ))}
        </div>
      </motion.div>
  ...
  if (step === "subject") {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pt-6 pb-4">
        ...
        <h2 className="text-xl font-bold text-foreground tracking-tight mb-0.5">{cat?.icon} {cat?.name}</h2>
        <p className="text-[13px] text-muted-foreground/60 mb-4">{t("chooseSubject")}</p>
        <StepBar step={step} />
        {subjects.length > 5 && (
        ...
        <div className="space-y-1.5">
          {filtered.map((sub, i) => (
            ...
            >
              ...
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-muted-foreground/15"><polyline points="9 18 15 12 9 6"/></svg>
            </motion.button>
          ))}
        </div>
      </motion.div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pt-6 pb-4">
      ...
      <h2 className="text-xl font-bold text-foreground tracking-tight mb-0.5">{t("details")}</h2>
      <p className="text-[13px] text-muted-foreground/60 mb-4">{t("describeTask")}</p>
      <StepBar step={step} />
      ...
      <div className="space-y-4">
        <div>
          <label className="text-[12px] font-semibold text-foreground mb-2 block">{t("topicLabel")}</label>
          <textarea value={topic} onChange={e => setTopic(e.target.value)}
            placeholder={t("topicPlaceholder")}
            className="input-field resize-none leading-relaxed" rows={4} />
          {topic.length > 0 && <div className="text-[10px] text-muted-foreground/40 mt-1.5 text-right tabular-nums">{topic.length}</div>}
        </div>
        <div>
          <label className="text-[12px] font-semibold text-foreground mb-2 block">{t("groupLabel")}</label>
      ...
      {!canGenerate && (
        <div className="mt-4 bg-destructive/5 border border-destructive/10 text-destructive rounded-xl p-3.5 text-[12px] flex items-center gap-2.5">
      ...
      <motion.button whileTap={{ scale: 0.98 }} onClick={handleGenerate}
        ...
        className="mt-6 w-full btn-primary py-[15px] font-semibold text-[15px] flex items-center justify-center gap-2.5">
