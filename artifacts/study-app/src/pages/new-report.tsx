...
export default function NewReport() {
  ...
  function reset() { setStep("type"); setReportType(""); setCategory(""); setSubject(""); setTopic(""); setGroup(""); setResult(""); setSearchQ(""); }
      ...
      <div className="px-5 pt-20 pb-4 flex flex-col items-center justify-center min-h-[70vh]">
        <div className="relative mb-10">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}
            ...
            style={{ border: "2.5px solid rgba(255,255,255,0.04)", borderTopColor: "#7c3aed", borderRightColor: "#06b6d4" }} />
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-[18px] font-black tabular gradient-text">{Math.round(progress)}%</span>
          </div>
        </div>
        <h2 className="text-xl font-bold text-white mb-2">{t("generating")}...</h2>
        <p className="text-[13px] text-white/20 mb-8">{t("generatingDesc")}</p>
        <div className="w-56 h-[3px] rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.04)" }}>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="px-5 pt-8 pb-4">
        <div className="flex items-center gap-4 mb-6">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", bounce: 0.5 }}
            className="w-14 h-14 rounded-2xl flex items-center justify-center glow-g"
          ...
          <div>
            ...
            <p className="text-[13px] text-white/25">{selType?.label} · {wordCount.toLocaleString()}</p>
          </div>
        </div>
        <div className="flex gap-3 mb-5">
          ...
          <motion.button whileTap={{ scale: 0.96 }} onClick={reset}
            className="flex-1 btn-ghost py-3.5 text-[14px] font-bold">{t("newOne")}</motion.button>
        </div>
  ...
  if (step === "error") {
    return (
      <div className="px-5 pt-20 pb-4 flex flex-col items-center justify-center min-h-[70vh]">
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6" style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.08)" }}>
        ...
        <p className="text-[14px] text-white/25 text-center mb-6">{errorMsg}</p>
        <button onClick={() => setStep("details")} className="btn-main px-8 py-3 text-[14px]">{t("tryAgain")}</button>
      </div>
    );
  }

  const colors = ["#a78bfa", "#67e8f9", "#34d399", "#fbbf24", "#f97316", "#f472b6"];

  if (step === "type") {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-5 pt-8 pb-4">
        <h2 className="text-[26px] font-black text-white tracking-tight mb-1">{t("newDocument")}</h2>
        ...
        <div className="space-y-2.5">
          {TYPES.map((type, i) => (
            <motion.button key={type.id}
              ...
              className="w-full g-card rounded-2xl p-4 text-left flex items-center gap-4 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-[1px] h-full" style={{ background: `linear-gradient(180deg, transparent, ${colors[i]}25, transparent)` }} />
              <div className="icon-box text-xl"
                style={{ background: `${colors[i]}0a`, border: `1px solid ${colors[i]}0d` }}>{type.icon}</div>
              <div className="flex-1">
                <div className="font-bold text-[15px] text-white">{type.label}</div>
                <div className="text-[11px] text-white/18 mt-0.5">{type.desc}</div>
              </div>
  ...
  if (step === "category") {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-5 pt-8 pb-4">
        ...
        <h2 className="text-[24px] font-black text-white tracking-tight mb-1">{t("chooseCategory")}</h2>
        <p className="text-[13px] text-white/20 mb-5">{CATS.reduce((a, c) => a + c.subjects.length, 0)}+ {t("allSubjects").toLowerCase()}</p>
        <StepBar step={step} />
        <div className="grid grid-cols-2 gap-3">
          {CATS.map((cat, i) => (
            <motion.button key={cat.id}
              ...
              className="g-card rounded-2xl p-4 text-left relative overflow-hidden">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[60%] h-[1px]"
                style={{ background: `linear-gradient(90deg, transparent, ${colors[i % 6]}20, transparent)` }} />
              <span className="text-[28px] block mb-2.5">{cat.icon}</span>
              <div className="font-black text-[13px] text-white leading-snug">{cat.name}</div>
              <div className="text-[10px] text-white/18 mt-1 font-semibold">{cat.subjects.length} {t("allSubjects").toLowerCase()}</div>
            </motion.button>
          ))}
        </div>
      </motion.div>
  ...
  if (step === "subject") {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-5 pt-8 pb-4">
        ...
        <h2 className="text-[24px] font-black text-white tracking-tight mb-1">{cat?.icon} {cat?.name}</h2>
        <p className="text-[13px] text-white/20 mb-5">{t("chooseSubject")}</p>
        <StepBar step={step} />
        {subs.length > 5 && (
        ...
        <div className="space-y-2">
          {filtered.map((sub, i) => (
            <motion.button key={sub.id}
              ...
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-white/6"><polyline points="9 18 15 12 9 6"/></svg>
            </motion.button>
          ))}
        </div>
      </motion.div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-5 pt-8 pb-4">
      ...
      <div className="space-y-4">
        <div>
          ...
          {topic.length > 0 && <div className="text-[11px] text-white/12 mt-1.5 text-right tabular">{topic.length}</div>}
        </div>
        <div>
      ...
      <motion.button whileTap={{ scale: 0.97 }} onClick={handleGenerate} disabled={!topic.trim() || !canGenerate}
        className="mt-6 w-full btn-main py-[18px] text-[15px] flex items-center justify-center gap-2.5">
