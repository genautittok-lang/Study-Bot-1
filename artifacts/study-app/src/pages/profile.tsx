...
export default function Profile() {
  ...
  const stagger = (i: number) => ({
    ...
    transition: { delay: i * 0.04, duration: 0.25, ease: [0.4, 0, 0.2, 1] as [number, number, number, number] },
  });
          ...
          {filteredLangs.map((lang, i) => {
            ...
            return (
              <motion.button key={lang.code}
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all ${active ? "bg-primary/6 border border-primary/12" : "active:bg-muted/50"}`}>
  ...
  return (
    <div className="px-4 pt-5 pb-4">
      <motion.h2 {...stagger(0)} className="text-lg font-bold text-foreground tracking-tight mb-4">{t("profileTitle")}</motion.h2>
      ...
      <motion.div {...stagger(1)} className="relative rounded-2xl p-5 mb-4 overflow-hidden">
        ...
        <div className="relative z-10 text-white">
          <div className="flex items-center gap-3.5 mb-4">
            <div className={`w-14 h-14 bg-gradient-to-br ${level.color} rounded-[18px] flex items-center justify-center shadow-lg text-xl font-black`}>
              {(user?.firstName || "S").charAt(0).toUpperCase()}
            </div>
            <div className="flex-1">
              <div className="text-[15px] font-bold">{user?.firstName || "Student"}</div>
              {user?.username && <div className="text-[12px] text-white/35">@{user.username}</div>}
              <div className="text-[10px] text-white/20 mt-0.5 tabular-nums">ID: {user?.telegramId}</div>
            </div>
          </div>
          <div className="bg-white/[0.06] border border-white/[0.04] rounded-xl p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[12px] font-medium text-white/50">{level.icon} {level.name}</span>
              {nextLvl && <span className="text-[10px] text-white/25">{nextLvl.reportsNeeded} → {nextLvl.name}</span>}
            </div>
      ...
      <motion.div {...stagger(2)} className="grid grid-cols-3 gap-2 mb-4">
        <div className="card-pro rounded-xl p-3 text-center">
          <div className="text-lg font-black text-foreground tabular-nums count-up">{total}</div>
          <div className="text-[10px] text-muted-foreground mt-0.5">{t("total")}</div>
        </div>
        <div className="card-pro rounded-xl p-3 text-center">
          <div className="text-lg font-black text-primary tabular-nums count-up">{balance}</div>
          <div className="text-[10px] text-muted-foreground mt-0.5">{t("availableReports")}</div>
        </div>
        <div className="card-pro rounded-xl p-3 text-center">
          <div className="text-lg font-black text-emerald-600 tabular-nums count-up">{referralCount}</div>
      ...
      <motion.div {...stagger(3)} className="mb-4">
        ...
        <div className="grid grid-cols-4 gap-2">
          {achievements.map((a, i) => (
            <div key={i} className={`card-pro rounded-xl p-2.5 text-center transition-all ${a.ok ? "" : "opacity-20"}`}>
              <div className={`w-8 h-8 mx-auto rounded-[10px] flex items-center justify-center mb-1.5 text-[11px] font-black ${a.ok ? "bg-primary/8 text-primary" : "bg-muted text-muted-foreground"}`}>
                {a.icon}
              </div>
      ...
      <motion.div {...stagger(4)} className="card-pro card-highlight rounded-2xl p-4 mb-4">
        <div className="flex items-start gap-3 mb-3">
          <div className="w-9 h-9 bg-emerald-500/8 rounded-xl flex items-center justify-center shrink-0">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="1.8"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" x2="19" y1="8" y2="14"/><line x1="22" x2="16" y1="11" y2="11"/></svg>
          </div>
          <div>
        ...
        <div className="bg-muted/40 rounded-xl p-3 mb-3">
          <div className="text-[10px] text-muted-foreground mb-0.5 uppercase tracking-wide">{t("referralCode")}</div>
          <div className="font-mono text-[15px] font-bold tracking-[0.1em] text-foreground">{referralCode}</div>
        </div>
        <motion.button whileTap={{ scale: 0.97 }} onClick={copyRef}
          className="w-full bg-emerald-600 text-white rounded-xl py-2.5 text-[13px] font-semibold flex items-center justify-center gap-2 shadow-sm shadow-emerald-600/15 active:bg-emerald-700 transition-colors">
          {copied ? (
      ...
      <motion.button {...stagger(5)} whileTap={{ scale: 0.98 }}
        ...
        <div className="w-9 h-9 bg-primary/6 rounded-xl flex items-center justify-center shrink-0">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--primary))" strokeWidth="1.8">
      ...
      <motion.div {...stagger(6)} className="text-center py-3">
        <p className="text-[10px] text-muted-foreground/30">StudyPro v1.0 · @studypro_support</p>
      </motion.div>
    </div>
  );
}
