  ...
  return (
    <div className="mb-3">
      ...
      <motion.button whileTap={{ scale: 0.97 }}
        ...
        style={{ background: "rgba(255,255,255,0.7)", border: "1.5px solid rgba(108,92,231,0.1)", backdropFilter: "blur(10px)" }}>
        ...
        <motion.div animate={{ scale: ok ? [1, 1.2, 1] : 1 }}
          ...
          style={{ background: ok ? "rgba(0,184,148,0.12)" : "rgba(108,92,231,0.08)" }}>
          {ok
    ...
    <div className="flex items-center gap-1.5 mb-5">
      {[1, 2, 3].map((i) => (
        <motion.div key={i} animate={{ opacity: i <= step ? 1 : 0.3, scale: i === step ? 1.1 : 1 }}
          ...
          style={{ background: i <= step ? "linear-gradient(90deg, #6C5CE7, #0984E3)" : "rgba(0,0,0,0.08)" }} />
      ))}
    </div>
  );
}
      ...
      {[...Array(8)].map((_, i) => (
        <motion.div key={i}
          ...
          style={{
            ...
            background: ["rgba(108,92,231,0.3)", "rgba(9,132,227,0.3)", "rgba(0,184,148,0.3)", "rgba(255,159,67,0.3)", "rgba(108,92,231,0.2)", "rgba(9,132,227,0.2)", "rgba(0,184,148,0.2)", "rgba(255,159,67,0.2)"][i],
  ...
  if (step === "done") {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative px-4 pt-14 pb-6 flex flex-col items-center min-h-[75vh] justify-center overflow-hidden">
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(108,92,231,0.08) 0%, transparent 70%)" }} />
        ...
        <motion.div
          ...
          <div className="w-24 h-24 rounded-[28px] flex items-center justify-center relative"
            style={{ background: "linear-gradient(135deg, rgba(0,184,148,0.12), rgba(0,184,148,0.06))", border: "2px solid rgba(0,184,148,0.2)" }}>
            <FloatingParticles />
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#00B894" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>
          </div>
          {[...Array(6)].map((_, i) => (
            <motion.div key={i}
              className="absolute w-2 h-2 rounded-full"
              style={{
                background: ["#6C5CE7", "#00B894", "#0984E3", "#FF9F43", "#6C5CE7", "#00B894"][i],
                top: "50%", left: "50%",
              }}
        ...
        {!isStars && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}
            ...
            style={{ background: "linear-gradient(135deg, rgba(108,92,231,0.06), rgba(9,132,227,0.04))", border: "1px solid rgba(108,92,231,0.1)" }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] font-bold text-[#6C5CE7]">{t("adminReviewing")}</span>
            </div>
        ...
        <motion.button whileTap={{ scale: 0.97 }} onClick={reset}
          ...
          style={{ background: "linear-gradient(135deg, #6C5CE7, #0984E3)", color: "white", boxShadow: "0 8px 20px rgba(108,92,231,0.3)" }}>
          {t("goBack")}
        </motion.button>
      </motion.div>
    );
      ...
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25, ease }}
        ...
        <div className="text-center mb-6">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", bounce: 0.4 }}
            ...
            style={{ background: "linear-gradient(135deg, rgba(108,92,231,0.12), rgba(9,132,227,0.08))", border: "1.5px solid rgba(108,92,231,0.15)" }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#6C5CE7" strokeWidth="1.8">
        ...
        <motion.button
          ...
          style={{
            background: screenshot ? "rgba(0,184,148,0.04)" : "rgba(108,92,231,0.03)",
            border: `2px dashed ${screenshot ? "rgba(0,184,148,0.35)" : screenshotError ? "rgba(239,68,68,0.4)" : "rgba(108,92,231,0.2)"}`,
            minHeight: 120,
          }}>
          {screenshot ? (
            <>
              ...
              <div className="text-center">
                <div className="text-[13px] font-bold text-[#00B894]">{t("screenshotUploaded")}</div>
                <div className="text-[10px] text-[#9ca3af] mt-0.5 max-w-[200px] truncate">{screenshotName}</div>
              </div>
              <div className="text-[11px] font-semibold text-[#6C5CE7]">{t("changeScreenshot")}</div>
            </>
          ) : (
            <>
              <div className="w-12 h-12 rounded-[14px] flex items-center justify-center"
                style={{ background: screenshotError ? "rgba(239,68,68,0.06)" : "rgba(108,92,231,0.07)" }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={screenshotError ? "#EF4444" : "#6C5CE7"} strokeWidth="1.8">
        ...
        <motion.button whileTap={{ scale: 0.97 }} onClick={submit} disabled={submitting}
          ...
          style={{
            background: screenshot ? "linear-gradient(135deg, #6C5CE7, #0984E3)" : "rgba(108,92,231,0.4)",
            color: "white",
            boxShadow: screenshot ? "0 8px 24px rgba(108,92,231,0.3)" : "none",
          }}>
          {submitting
      ...
      <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.22, ease }}
        ...
        <div className="rounded-[22px] p-4 mb-4 relative overflow-hidden"
          ...
          <div className="absolute bottom-0 left-0 w-28 h-28 rounded-full opacity-5"
            style={{ background: "radial-gradient(circle, #6C5CE7, transparent 70%)", transform: "translate(-20%, 20%)" }} />
        ...
        {!isStars && (
          <div className="rounded-[16px] p-3.5 mb-4" style={{ background: "rgba(108,92,231,0.03)", border: "1px solid rgba(108,92,231,0.07)" }}>
            <div className="flex items-center gap-2 mb-2.5">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#6C5CE7" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><path d="M12 8h.01" /></svg>
              <span className="text-[10px] font-bold text-[#6C5CE7] uppercase tracking-wider">{t("howItWorks")}</span>
            </div>
            {[
        ...
        {!isStars && (
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => { hapticFeedback("light"); setStep("screenshot"); }}
            className="w-full py-[15px] rounded-[18px] text-[14px] font-bold flex items-center justify-center gap-2"
            style={{ background: "linear-gradient(135deg, #6C5CE7, #0984E3)", color: "white", boxShadow: "0 8px 24px rgba(108,92,231,0.28)" }}>
  ...
  return (
    <div className="px-4 pt-5 pb-6">
      <motion.div {...a(0)} className="relative rounded-[28px] p-5 mb-5 overflow-hidden"
        style={{
          background: "linear-gradient(145deg, #0984E3, #0773C7, #6C5CE7)",
          boxShadow: "0 16px 48px rgba(9,132,227,0.35), 0 4px 12px rgba(108,92,231,0.2)",
        }}>
              ...
              <div className="flex-1 text-left">
                <div className="font-bold text-[14px] leading-tight flex items-center gap-2">
                  ...
                  <span className="text-[8px] font-bold px-2 py-0.5 rounded-full"
                    style={{ background: "linear-gradient(135deg, rgba(108,92,231,0.12), rgba(9,132,227,0.08))", color: "#6C5CE7" }}>
                    {t("popular")}
                  </span>
                </div>
                <div className="text-[11px] text-[#9ca3af] mt-0.5 font-medium">{t("cardPaymentDesc")}</div>
                <div className="text-[10px] text-[#6C5CE7] mt-0.5 font-bold">250 UAH · 15 {t("reportsAvailable").toLowerCase()}</div>
              </div>
        ...
        <motion.div {...a(isUkraine ? 4 : 3)}>
          <motion.button whileTap={{ scale: 0.97 }}
            ...
            <div className="flex-1 text-left">
              ...
              <div className="text-[10px] text-[#00B894] mt-0.5 font-bold">5 USDT · 15 {t("reportsAvailable").toLowerCase()}</div>
            </div>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg>
          </motion.button>
        </motion.div>
      ...
      <motion.div {...a(isUkraine ? 5 : 4)} className="mt-4 rounded-[14px] p-3 flex items-start gap-2.5"
        style={{ background: "rgba(108,92,231,0.03)", border: "1px solid rgba(108,92,231,0.07)" }}>
