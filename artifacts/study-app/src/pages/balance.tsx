...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="mb-3">
      <div className="text-[9px] text-white/15 font-semibold tracking-[0.1em] uppercase mb-1.5">{label}</div>
      <motion.button whileTap={{ scale: 0.98 }}
        ...
        <code className="text-[12px] font-mono text-white/80 flex-1 text-left break-all select-all">{value}</code>
        <div className="shrink-0 w-7 h-7 rounded-lg flex items-center justify-center"
          style={{ background: ok ? "rgba(16,185,129,0.06)" : "rgba(255,255,255,0.02)" }}>
          {ok ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#34d399" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
            : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-white/15"><rect width="14" height="14" x="8" y="8" rx="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>}
        </div>
      </motion.button>
    </div>
  );
  ...
  return (
    <div className="hero-card hero-shimmer p-4 mb-5">
      ...
      <div className="relative z-10 flex items-center justify-between">
        <div>
          <div className="text-[9px] text-white/15 font-semibold tracking-[0.1em] uppercase mb-1.5">{t("paymentAmount")}</div>
          <div className="flex items-baseline gap-2">
            <span className="text-[38px] font-extrabold text-white tabular tracking-tighter leading-none">15</span>
            <span className="text-[12px] text-white/15 font-medium">{t("reportsAvailable").toLowerCase()}</span>
          </div>
        </div>
        <div className="w-10 h-10 rounded-xl flex items-center justify-center"
          style={{ background: "rgba(124,58,237,0.06)" }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="1.5"><path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"/></svg>
        </div>
      </div>
    </div>
  );
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", bounce: 0.5, duration: 0.6 }}
          ...
          style={{ background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.08)" }}>
        ...
        <p className="text-[13px] text-white/20 text-center max-w-[260px] mb-6">{t("paymentPending")}</p>
  ...
  if (method === "card") {
    return (
      <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25, ease }} className="px-5 pt-7 pb-4">
        ...
        <p className="text-[12px] text-white/18 mb-5">{t("transferDescription")}</p>
        <PkgCard />
        <CopyField label={t("cardNumber")} value={CARD} />
        <div className="g-card-s rounded-xl p-3.5 flex items-start gap-2.5 mb-2.5">
          ...
          <p className="text-[10px] text-white/18 leading-relaxed">{t("cardNote")} <span className="font-mono font-semibold text-[#67e8f9]">{user?.telegramId}</span></p>
        </div>
        <div className="g-card-s rounded-xl p-3.5 flex items-start gap-2.5" style={{ background: "rgba(251,191,36,0.02)", borderColor: "rgba(251,191,36,0.06)" }}>
          ...
          <p className="text-[10px] text-white/20 leading-relaxed">{t("receiptNote")}</p>
        </div>
  ...
  if (method === "crypto") {
    return (
      <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25, ease }} className="px-5 pt-7 pb-4">
        ...
        <p className="text-[12px] text-white/18 mb-5">USDT · TRC-20</p>
        <PkgCard />
  ...
  if (method === "stars") {
    return (
      <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25, ease }} className="px-5 pt-7 pb-4">
        ...
        <p className="text-[12px] text-white/18 mb-5">{t("starsDesc")}</p>
        <PkgCard />
        <div className="g-card-s rounded-xl p-4 mb-5">
          <p className="text-[13px] text-white/40 leading-relaxed mb-2">{t("starsInstructions")}</p>
          <p className="text-[11px] text-white/20">{t("supportContact")}: <span className="font-semibold text-[#a78bfa]">@studypro_support</span></p>
        </div>
        <motion.button whileTap={{ scale: 0.97 }} onClick={confirm} disabled={confirming}
  ...
  return (
    <div className="px-5 pt-7 pb-4">
      <motion.div {...st(0)}>
        ...
        <p className="text-[11px] text-white/18 mb-5">{t("topUpBalance")}</p>
      </motion.div>
      ...
      <motion.div {...st(1)} className="hero-card hero-shimmer p-5 mb-5">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-0.5">
            <span className="text-[9px] text-white/15 font-semibold tracking-[0.12em] uppercase">{t("currentBalance")}</span>
            {user?.totalReports !== undefined && <span className="text-[9px] text-white/8 font-medium">{t("total")}: {user.totalReports}</span>}
          </div>
          <div className="flex items-end gap-2.5 mt-1">
            ...
            <span className="text-[12px] text-white/10 pb-3 font-medium">{t("reportsAvailable")}</span>
          </div>
          {!user?.freeReportsUsed && (
            <div className="mt-2.5 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg"
              style={{ background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.06)" }}>
      ...
      <motion.div {...st(2)}>
        <motion.button whileTap={{ scale: 0.97 }} onClick={payStars} disabled={starsLoading}
          ...
          style={{
            ...
            boxShadow: "0 0 0 1px rgba(251,191,36,0.1), 0 4px 16px -4px rgba(217,119,6,0.25)",
          }}>
      ...
      <div className="space-y-2">
        {[
          ...
          { m: "crypto" as Method, title: t("cryptoPayment"), price: "5 USDT (TRC-20)", color: "#34d399",
            icon: <span className="text-base font-bold" style={{ color: "#34d399" }}>₮</span> },
        ].map((item, i) => (
          <motion.button key={item.m} {...st(4 + i)} whileTap={{ scale: 0.97 }}
            ...
            <div className="icon-box" style={{ background: `${item.color}08` }}>
              {item.icon}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-white/15 mt-0.5 font-medium">{item.price}</div>
            </div>
      ...
      <motion.div {...st(8)} className="mt-5 g-card-s rounded-xl p-3.5 flex items-start gap-2.5">
        ...
        <p className="text-[10px] text-white/15 leading-relaxed">{t("paymentNote")}</p>
      </motion.div>
    </div>
  );
}
