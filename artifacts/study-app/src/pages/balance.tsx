...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="mb-3">
      <div className="text-[9px] text-white/18 font-semibold tracking-[0.1em] uppercase mb-1.5">{label}</div>
      <motion.button whileTap={{ scale: 0.98 }}
        ...
        <code className="text-[12px] font-mono text-white/45 flex-1 text-left break-all select-all">{value}</code>
        <div className="shrink-0 w-7 h-7 rounded-[10px] flex items-center justify-center transition-colors"
          style={{ background: ok ? "rgba(16,185,129,0.08)" : "rgba(255,255,255,0.04)" }}>
          {ok
            ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
            : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-white/22"><rect width="14" height="14" x="8" y="8" rx="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>}
        </div>
      </motion.button>
    </div>
  );
...
function PkgCard() {
  return (
    <div className="hero-card hero-shimmer p-4 mb-4">
      <div className="shimmer-bar" />
      <div className="relative z-10 flex items-center justify-between">
        <div>
          <div className="text-[9px] text-white/18 font-semibold tracking-[0.1em] uppercase mb-1.5">{t("paymentAmount")}</div>
          <div className="flex items-baseline gap-2">
            <span className="text-[36px] font-extrabold text-white tabular tracking-[-0.04em] leading-none num-glow">15</span>
            <span className="text-[11px] text-white/18 font-medium">{t("reportsAvailable").toLowerCase()}</span>
          </div>
        </div>
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", bounce: 0.5, duration: 0.6 }}
          ...
          style={{ background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.12)", boxShadow: "0 0 28px rgba(16,185,129,0.08)" }}>
        ...
        <p className="text-[13px] text-white/28 text-center max-w-[260px] mb-6">{t("paymentPending")}</p>
  ...
  if (method === "card") {
    return (
      <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, ease }} className="px-4 pt-6 pb-4">
        ...
        <p className="text-[11px] text-white/25 mb-4">{t("transferDescription")}</p>
        <PkgCard />
        <CopyField label={t("cardNumber")} value={CARD} />
        <div className="g-card rounded-[14px] p-3 flex items-start gap-2.5 mb-2.5">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#7dd3fc" strokeWidth="1.5" className="mt-0.5 shrink-0"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="16" y2="12"/><line x1="12" x2="12.01" y1="8" y2="8"/></svg>
          <p className="text-[10px] text-white/28 leading-relaxed">{t("cardNote")} <span className="font-mono font-semibold text-[#7dd3fc]">{user?.telegramId}</span></p>
        </div>
        <div className="g-card rounded-[14px] p-3 flex items-start gap-2.5" style={{ borderColor: "rgba(251,191,36,0.08)" }}>
          ...
          <p className="text-[10px] text-white/22 leading-relaxed">{t("receiptNote")}</p>
        </div>
  ...
  if (method === "crypto") {
    return (
      <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, ease }} className="px-4 pt-6 pb-4">
        ...
        <p className="text-[11px] text-white/25 mb-4">USDT · TRC-20</p>
        <PkgCard />
  ...
  if (method === "stars") {
    return (
      <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, ease }} className="px-4 pt-6 pb-4">
        ...
        <p className="text-[11px] text-white/25 mb-4">{t("starsDesc")}</p>
        <PkgCard />
        <div className="g-card rounded-[14px] p-3.5 mb-4">
          <p className="text-[12px] text-white/35 leading-relaxed mb-2">{t("starsInstructions")}</p>
          <p className="text-[11px] text-white/22">{t("supportContact")}: <span className="font-semibold text-[#b8a4ff]">@studypro_support</span></p>
        </div>
        <motion.button whileTap={{ scale: 0.97 }} onClick={confirm} disabled={confirming}
  ...
  return (
    <div className="px-4 pt-6 pb-4">
      <motion.div {...a(0)}>
        ...
        <p className="text-[11px] text-white/22 mb-4">{t("topUpBalance")}</p>
      </motion.div>

      {/* Balance hero card */}
      <motion.div {...a(1)} className="hero-card hero-shimmer p-5 mb-4">
        <div className="shimmer-bar" />
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[9px] text-white/18 font-semibold tracking-[0.12em] uppercase">{t("currentBalance")}</span>
            {user?.totalReports !== undefined && <span className="text-[9px] text-white/12 font-medium tabular">{t("total")}: {user.totalReports}</span>}
          </div>
          <div className="flex items-end gap-2.5 mt-1">
            <span className="text-[50px] font-extrabold tabular leading-none tracking-[-0.05em] num-glow">{bal}</span>
            <span className="text-[11px] text-white/14 pb-2.5 font-medium">{t("reportsAvailable")}</span>
          </div>
      ...
      <motion.div {...a(2)}>
        <motion.button whileTap={{ scale: 0.97 }} onClick={payStars} disabled={starsLoading}
          className="w-full rounded-[18px] py-[14px] px-5 font-bold text-[14px] flex items-center justify-center gap-2.5 mb-4 text-white relative overflow-hidden"
          style={{
            background: "linear-gradient(135deg, rgba(180,83,9,0.25), rgba(245,158,11,0.15))",
            border: "1px solid rgba(245,158,11,0.15)",
            boxShadow: "0 0 24px rgba(245,158,11,0.06), 0 1px 0 rgba(255,255,255,0.04) inset",
            backdropFilter: "blur(30px)",
          }}>
          {starsLoading
            ? <div className="spinner w-5 h-5 border-white/20 border-t-amber-400" />
      ...
      <div className="space-y-2">
        {[
          { m: "card" as Method, title: t("cardPayment"), price: "250 UAH", tag: t("popular"), color: "#b8a4ff",
            icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#b8a4ff" strokeWidth="1.5"><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/></svg> },
          ...
          <motion.button key={item.m} {...a(4 + i)} whileTap={{ scale: 0.97 }}
            ...
            <div className="icon-box" style={{ background: `${item.color}0a` }}>
              {item.icon}
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-[13px] text-white/88 flex items-center gap-2 leading-tight">
                {item.title}
                {item.tag && <span className="badge-g text-[8px] py-0.5 px-2">{item.tag}</span>}
              </div>
              <div className="text-[10px] text-white/22 mt-0.5 font-medium">{item.price}</div>
            </div>
      ...
      <motion.div {...a(7)} className="mt-4 g-card rounded-[14px] p-3 flex items-start gap-2.5">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#7dd3fc" strokeWidth="1.5" className="shrink-0 opacity-35 mt-0.5"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="16" y2="12"/><line x1="12" x2="12.01" y1="8" y2="8"/></svg>
        <p className="text-[10px] text-white/18 leading-relaxed">{t("paymentNote")}</p>
      </motion.div>
    </div>
  );
}
