  ...
  if (selected) {
    ...
    return (
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, ease }} className="px-5 pt-8 pb-4">
        ...
        <h2 className="text-[18px] font-black text-white leading-snug text-balance mb-3">{selected.topic}</h2>
        <div className="flex items-center gap-2 flex-wrap mb-5">
          ...
          {wc > 0 && <span className="text-[11px] text-white/15 tabular font-semibold">{wc.toLocaleString()}</span>}
        </div>
        <motion.button whileTap={{ scale: 0.96 }}
          onClick={() => { navigator.clipboard.writeText(selected.content || ""); hapticSuccess(); }}
          className="btn-main px-5 py-3 text-[14px] flex items-center gap-2 mb-5">
  ...
  return (
    <div className="px-5 pt-8 pb-4">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ease }}>
        <h2 className="text-[26px] font-black text-white tracking-tight mb-1">{t("history")}</h2>
        <p className="text-[13px] text-white/20 mb-7">{reports.length} {t("total").toLowerCase()}</p>
      </motion.div>
      {loading ? (
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="g-card-s rounded-2xl p-4 flex gap-3.5 items-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease }}
          ...
          <div className="relative mb-6">
            <div className="w-24 h-24 rounded-3xl flex items-center justify-center"
              ...
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1" opacity="0.12">
                <path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>
            </div>
            <motion.div animate={{ scale: [1, 1.3, 1], opacity: [0.04, 0.1, 0.04] }}
              transition={{ duration: 4, repeat: Infinity }}
              className="absolute -inset-6 -z-10"
              style={{ background: "radial-gradient(circle, rgba(124,58,237,0.06), transparent 70%)", borderRadius: "40px" }} />
          </div>
          <h3 className="text-lg font-bold text-white mb-2">{t("noHistory")}</h3>
          <p className="text-[13px] text-white/18 max-w-[240px]">{t("noHistoryDesc")}</p>
        </motion.div>
      ) : (
            ...
            {reports.map((r, i) => (
              <motion.button key={r.id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-white/18 mt-0.5 flex items-center gap-1.5">
